...
// Create DTO that matches the actual database schema
interface CreatePassengerDto {
  first_name: string;
  last_name: string;
  email?: string;
  phone?: string;
...
export const PassengerCreateForm: React.FC<PassengerCreateFormProps> = ({ onClose, onSuccess }) => {
  const { success, error } = useToast();
  const [loading, setLoading] = useState(false);
  
  const [formData, setFormData] = useState<CreatePassengerDto>({
    first_name: '',
    last_name: '',
    email: '',
    phone: '',
  ...
  const [errors, setErrors] = useState<Partial<CreatePassengerDto>>({});

  const validateForm = (): boolean => {
    const newErrors: Partial<CreatePassengerDto> = {};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await passengersApi.create(createData);
      console.log('✅ Passenger created:', response.data);
      
      success(
        'Passenger Created', 
        `Passenger ${formData.first_name} ${formData.last_name} has been successfully created`
      );
      ...
      error('Failed to create passenger', errorMessage);
    } finally {
      setLoading(false);
    }
  };

  const handleInputChange = (field: keyof CreatePassengerDto, value: string) => {
