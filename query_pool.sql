-- Пул запросов для операций с базой данных системы управления аэропортом
-- Простые операции над данными с европейскими данными
...
WHERE dep_airport.iata_code = 'SVO'  -- Москва
AND arr_airport.iata_code = 'LED'     -- Санкт-Петербург
...
-- Параметры: first_name, last_name, passport_number, nationality, date_of_birth, phone, email
...
-- 8. Покупка билета
-- Параметры: flight_id, passenger_id, seat_number, class, price
...
-- 9. Регистрация багажа
-- Параметры: passenger_id, flight_id, weight
...
-- 10. Назначение экипажа на рейс
-- Параметры: flight_id, user_id, position
INSERT INTO Flight_Crew (flight_id, user_id, position)
VALUES (7, 3, 'pilot')
RETURNING crew_id;

-- 11. Добавление записи технического обслуживания
-- Параметры: aircraft_id, maintenance_type, description, start_date, cost, technician_id
...
-- 13. Обновление статуса рейса
-- Параметры: flight_id, new_status
UPDATE Flights 
SET status = 'boarding',
...
-- 14. Регистрация на рейс (check-in)
-- Параметры: ticket_id, seat_number
UPDATE Tickets 
...
-- 15. Обновление статуса багажа
-- Параметры: baggage_id, new_status
UPDATE Baggage 
SET status = 'loaded',
    delivery_time = CASE 
...
-- 16. Завершение технического обслуживания
-- Параметры: maintenance_id, end_date, final_cost
...
-- 17. Обновление информации о пользователе
-- Параметры: user_id, phone, email
UPDATE Users 
...
-- 18. Обновление информации о пассажире
UPDATE Passengers 
...
-- 20. Удаление записи технического обслуживания (только если не завершено)
...
-- 21. Удаление пассажира (только если нет активных билетов)
-- Параметры: passenger_id
...
-- 23. Топ-5 самых загруженных рейсов
SELECT 
    f.flight_number,
...
-- 26. Доходы по классам обслуживания
SELECT 
    t.class,
...
-- 28. Статистика по европейским пассажирам
SELECT 
    p.nationality,
...
-- 32. Проверка целостности данных
SELECT 
...
-- 36. Использование функции поиска рейсов
SELECT * FROM search_flights('SVO', 'LED', '2024-12-21');

-- 37. Проверка доступности места с помощью функции
SELECT 
    f.flight_number,
    '15A' as seat_number,
...
-- 38. Расчет загрузки рейса с помощью функции
SELECT 
    f.flight_number,
    calculate_flight_load(f.flight_id) as load_percentage,
    CASE 
...
-- 39. Поиск рейсов в европейские города
SELECT 
    f.flight_number,
    f.scheduled_departure,
    f.price,
...
-- 40. Статистика по европейским маршрутам
SELECT 
